...
RUN apt-get update \
  && apt-get install --no-install-recommends --no-install-suggests -qy nginx ca-certificates openssl build-essential python3 python3-dev python3-setuptools python3-pip \
    git gcc libpq-dev libffi-dev libssl-dev uwsgi uwsgi-plugin-python3 \
